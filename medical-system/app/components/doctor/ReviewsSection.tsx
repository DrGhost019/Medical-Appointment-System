...
// تابع داخلی برای تبدیل ساده اعداد انگلیسی به فارسی
...
// تعریف اینترفیس منطبق با ساختار دیتای کامنت‌ها
...
const ReviewsSection = ({ doctorId, doctorName, reviews = [], rating = 5, reviewCount = 0 }: ReviewsSectionProps) => {
  const [showAll, setShowAll] = useState(false);

  // لایه محافظ برای جلوگیری از کرش در صورت فرستاده نشدن آرایه
    ...
    <div className="w-[804px] rounded-[10px] border border-[#E7E7E7] bg-white p-6 flex flex-col gap-6" dir="rtl">
      ...
      <div className="flex items-center justify-between border-b border-[#E7E7E7] pb-4">
        ...
        <Link href={`/doctors/${doctorId}/review`}>
          <button type="button" className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#4179F0] text-[#4179F0] font-vazirmatn font-medium text-sm hover:bg-blue-50 transition-colors">
      ...
      <div className="flex flex-col gap-6 text-right">
        {displayedReviews.length === 0 ? (
          ...
          displayedReviews.map((review, idx) => {
            ...
            const userAvatar = review.userImage || '/assets/default-avatar.png';
      ...
      {/* دکمه نظرات بیشتر */}
      {!showAll && safeReviews.length > 3 && (
        <div className="flex justify-center pt-2">
          <button
            type="button"
            onClick={() => setShowAll(true)}
            className="flex items-center gap-2 text-[#4179F0] font-vazirmatn font-medium text-sm hover:text-[#3565d0] transition-colors"
          >
            نظرات بیشتر
            <ChevronDown size={16} />
          </button>
